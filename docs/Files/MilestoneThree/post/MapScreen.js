...
import { rawShoelaceArea } from '../utils/sharedGeometry';
import { metersPerDegree } from '../utils/geometryUtils';
...
const calculateCentroid = (coordinates) => {
  ...
  if (Array.isArray(coordinates[0]) && Array.isArray(coordinates[0][0]) && Array.isArray(coordinates[0][0][0])) {
    ...
    coordinates.forEach(poly => {
      const area = rawShoelaceArea(poly[0]);
      if (area > maxArea) {
        maxArea = area;
        ring = poly[0];
      }
...
const toLocalXY = (pins) => {
  ...
  const xy = pins.map((p) => {
    const x = (p.longitude - lng0) * m.lng;
    const y = (p.latitude - lat0) * m.lat;
    return { x, y };
  });
