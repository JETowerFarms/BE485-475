...
const COUNTY_ADJACENCY = buildAdjacencyList(
  COUNTY_POLYGONS,
  (c) => c.name,
  (c) => c.polygon,
  (p1, p2) => polygonsAdjacentXY(p1, p2, 1),
);
const COUNTY_COLOR_MAP = welshPowellColor(
  COUNTY_POLYGONS.map((c) => c.name),
  COUNTY_ADJACENCY,
  MAP_COLORS.length,
);
validateColoring(
...
const findCountyAtPoint = (svgX, svgY) => {
  for (const county of COUNTY_POLYGONS) {
    if (pointInPolygonXY({ x: svgX, y: svgY }, county.polygon)) {
      return county;
    }
  }
  return null;
