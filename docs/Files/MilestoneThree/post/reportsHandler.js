...
function buildGridPoints(coordinates) {
  geo.validateCoordinates(coordinates);

  const normalizedCoordinates = geo.isLikelyLatLng(coordinates)
    ? geo.swapPairs(coordinates)
    : coordinates;
  ...
  const ring = geo.closeRing(normalizedCoordinates);
  ...
  if (gridPoints.length === 0) {
    const reorderedRing = geo.reorderPolygon(coordinates);
    ...
    const swapped = geo.swapPairs(coordinates);
    const swappedRing = geo.closeRing(swapped);
    ...
    const swappedReorderedRing = geo.reorderPolygon(swapped);
  ...
  try {
    ...
    const insideResults = parserResult.results
      .map((result, index) => {
        ...
        return geo.pointInPolygon(coord, ring) ? result : null;
      })
      .filter(Boolean);
    ...
    const medianSuitability = geo.computeMedian(overallScores);
    const trimmedMeanSuitability = geo.computeTrimmedMean(overallScores, 0.1);
    const insideMedianSuitability = geo.computeMedian(insideScores);
...
function calculateFarmArea(coordinates) {
  const acres = geo.polygonAreaAcres(coordinates);
