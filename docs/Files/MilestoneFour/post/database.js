...
const queries = {
  ...
  getFarmAnalysis: async (farmId) => {
    return db.oneOrNone(
      `SELECT farm_id, total_points, avg_overall, avg_land_cover, avg_slope,
              avg_transmission, avg_population, min_score, max_score,
              suitable_area_acres, analysis_data, created_at
       FROM farm_analysis
       WHERE farm_id = $1`,
      [farmId]
    );
  ...
  // ── Farm landcover reports ────────────────────────────────────────
  // Table is created by DATABASE_SCHEMA.sql; no runtime DDL needed.
