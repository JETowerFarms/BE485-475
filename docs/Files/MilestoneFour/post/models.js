...
const MODEL_COLUMNS = ['id', 'name', 'description', 'equations', 'created_at', 'updated_at',
  ...FIELD_DEFS.map(f => f.column)].join(', ');

router.get('/', async (req, res) => {
  try {
    const rows = await db.any(`SELECT ${MODEL_COLUMNS} FROM models ORDER BY created_at DESC`);
...
router.get('/template', async (req, res) => {
  try {
    const row = await db.oneOrNone(`SELECT ${MODEL_COLUMNS} FROM models WHERE name='Default' ORDER BY id ASC LIMIT 1`);
    if (row) {
