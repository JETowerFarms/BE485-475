...
async function fetchModel(modelId) {
  ...
  const MODEL_COLUMNS = [
    'id', 'name', 'description',
    ...Object.keys(DEFAULT_MODEL_CONFIG).filter(k => k !== 'name'),
    'equations', 'created_at', 'updated_at'
  ].join(', ');
  return db.oneOrNone(`SELECT ${MODEL_COLUMNS} FROM models WHERE id = $1`, [modelId]);
}
...
router.get('/incentives/all', async (_req, res) => {
  try {
    const INCENTIVE_ALL_COLUMNS = [
      'id', 'name', 'category', 'description', 'eligibility', 'incentive_group',
      'itc_override', 'itc_bonus', 'ptc_per_kwh', 'ptc_years',
      'capex_grant_fraction', 'capex_grant_cap', 'capex_flat_reduction',
      'opex_savings_per_mw_yr', 'depreciation_100pct', 'depreciation_tax_rate',
      'rec_per_mwh', 'rec_years', 'community_payment_per_mw',
      'farmer_cost_share_per_acre', 'farmer_annual_revenue_per_acre',
      'farmer_annual_cost_per_acre', 'configurable',
      'mutual_exclusion_group', 'requires_group', 'active', 'sort_order'
    ].join(', ');
    const rows = await db.any(
      `SELECT ${INCENTIVE_ALL_COLUMNS} FROM incentives ORDER BY sort_order, id`
    );
...
// POST /incentives – create a new incentive (admin only)
...
// PUT /incentives/:id – update an existing incentive (admin only)
...
// DELETE /incentives/:id – soft-delete (set active=false) (admin only)
