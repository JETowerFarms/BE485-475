...
def compute_annual_farmer_cashflows(
    A_s: float,
    A_c_by_crop: Dict[str, float],
    setback_crop_acres: float,
    L_solar: float,
    lease: LeaseParameters,
    crops: List[CropParameters],
    econ: EconomicParameters,
    farmer: FarmerParameters,
    eqip_benefit: float = 0.0,
) -> List[Dict[str, Any]]:
    """Compute year-by-year farmer cashflows and cumulative NPV.

    Returns a list of dicts (one per year 0..T) with:
      year, lease_income, crop_income, other_income, total_income,
      discount_factor, discounted_cashflow, cumulative_npv
    """
    T = econ.project_life
    df = econ.discount_factors
    table = []
    ...
    # Setback buffer acres distributed evenly across crops (mirrors LP avg_pv_crop)
    n_crops = len(crops) if crops else 1
    setback_per_crop = setback_crop_acres / n_crops if n_crops > 0 else 0.0
    ...
    for t in range(T + 1):
        ...
        if t >= 1:
            # Lease income with escalation
            if A_s > 0:
                lease_income = L_solar * ((1 + lease.escalation_rate) ** t) * A_s

            # Crop income from traditional acres + setback buffer
            for crop in crops:
                a_c = A_c_by_crop.get(crop.name, 0.0) + setback_per_crop
                if a_c > 0:
                    price = crop.price_crop(t, econ)
                    revenue = crop.yield_per_acre * price
                    net = revenue - crop.cost_per_acre + farmer.pa116_credit_per_acre
        ...
        if t == 0 and eqip_benefit > 0:
            other_income = eqip_benefit

        total_income = lease_income + crop_income + other_income
        discounted = total_income * float(df[t])
        cumulative_npv += discounted

        table.append({
            'year': t,
            'lease_income': round(lease_income, 2),
            'crop_income': round(crop_income, 2),
            'other_income': round(other_income, 2),
            'total_income': round(total_income, 2),
            'discount_factor': round(float(df[t]), 6),
...
def optimize_land_allocation(
    ...
    # ── Compute annual farmer cashflow table ─────────────────────────────
    annual_npv_table = compute_annual_farmer_cashflows(
        A_s=A_s_opt,
        A_c_by_crop={crop.name: A_c_opts[i] for i, crop in enumerate(crops)},
        setback_crop_acres=setback_crop_acres,
        L_solar=L_solar,
        lease=lease,
        crops=crops,
        econ=econ,
    ...
    result = {
        ...
        "annual_npv_table": annual_npv_table,
    }
    return result
...
def evaluate_scenario(
    ...
    for crop in crops:
        ...
        # Update annual NPV table year 0 with EQIP benefit
        if eqip_bonus > 0 and result.get('annual_npv_table'):
            yr0 = result['annual_npv_table'][0]
            yr0['other_income'] = round(eqip_bonus, 2)
            yr0['total_income'] = round(yr0['lease_income'] + yr0['crop_income'] + eqip_bonus, 2)
            yr0['discounted_cashflow'] = round(yr0['total_income'] * yr0['discount_factor'], 2)
            # Recompute cumulative NPV from year 0 onward
            cum = 0.0
            for row in result['annual_npv_table']:
                cum += row['discounted_cashflow']
                row['cumulative_npv'] = round(cum, 2)
