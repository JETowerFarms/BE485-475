...
const calculateAreaAcres = (coordinates) => {
  ...
  // Convert lat/lng to local planar coordinates
  const lat0 = coordinates.reduce((acc, coord) => acc + coord[1], 0) / coordinates.length;
  const lng0 = coordinates.reduce((acc, coord) => acc + coord[0], 0) / coordinates.length;
  const latRad = (lat0 * Math.PI) / 180;
  const metersPerDegLat = 111320; // meters per degree latitude
  const metersPerDegLng = 111320 * Math.cos(latRad); // meters per degree longitude

  const xy = coordinates.map(coord => ({
    x: (coord[0] - lng0) * metersPerDegLng,
    y: (coord[1] - lat0) * metersPerDegLat,
  }));
